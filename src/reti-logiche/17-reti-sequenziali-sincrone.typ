...
Per campionare con periodo $T_0$, si possono utilizzare $k$ flip-flop D (tanti quanti i bit di stato da memorizzare), il cui ingresso $"CLK"$ è collegato ad un segnale periodico di clock, con periodo $T_0$, che posso generare da un circuito oscillatore, con frequenza $1 / T_0$.
...
- Al fronte di salita del segnale di clock, vengono campionati i bit di stato futuro dai FF-D
- Dopo un tempo pari al tempo di risposta dei FF-D, lo stato futuro diventa stato presente
...
- I bit di stato futuro $Y_(0 dots k-1)$ devono rimanere costanti sia durante $t_(s u)$, sia durante $t_h$
...
- $"A_RESET"$ e $"A_RES" arrow$ Segnali di reset asincroni
- $"RESET"$ e $"RES" arrow$ Segnali di reset sincroni
...
- Comprensione delle specifiche e individuazione del grafo degli stati
- Definizione della tabella di flusso a partire dal grafo degli stati
- Codifica degli stati e definizione della tabella delle transizioni
- Sintesi della parte combinatoria
- Disegno dello schema logico
