...
Questo si ottiene a livello elettronico, facendo si che la relazione ingresso-uscita analogica del circuito combinatorio $f_(r c)$, che poniamo in retroazione, abbia tratti a pendenza minore di 1 intorno ad H e L.
...
#table(
  columns: 6,
  [*Comando*], [*Ingresso $C$*], [*Ingresso $D$*], [*Uscita $Q$*], [*Ingresso $S$*], [*Ingresso $R$*],
...
L'ingresso $D$ deve essere costante durante $t_(s u)+t_h$ per garantire il corretto funzionamento del latch.
...
- *Tempo di set-up $t_(s u)$*: tempo minimo in cui $D$ deve essere costante prima del fronte
...
Come nel latch CD, $D$ deve rimanere costante sia durante $t_(s u)$, sia durante $t_h$.
