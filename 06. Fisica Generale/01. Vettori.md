...
![](somma-di-vettori.png)

Regola del parallelogramma:

![](regola-del-parallelogramma.png)
...
![](vettori-e-coordinate-una-dimensione.png)

- In due dimensioni, un sistema di riferimento è dato da due versori ortogonali:

![](vettori-e-coordinate-due-dimensioni.png)
...
![](vettori-e-coordinate-tre-dimensioni.png)
...
- *Somma*: $\vec{a} = (a_x, a_y, a_z), \vec{b} = (b_x, b_y, b_z)$

$$
\vec{a} + \vec{b} = (a_x+b_x, a_y+b_y, a_z+b_z)
$$

- *Prodotto per scalare*: $\lambda \in \mathbb{R}, \vec{a} \in \mathbb{R}^3, \vec{a} = (a_x, a_y, a_z)$

$$
\lambda \vec{a} = (\lambda a_x, \lambda a_y, \lambda a_z)
$$

- *Prodotto scalare*: $\vec{a} = a_i \hat{i} + a_j \hat{j} + a_k \hat{k}, \vec{b} = b_i \hat{i} + b_j \hat{j} + b_k \hat{k}$
...
- *Prodotto vettoriale*: $\vec{a} = a_i \hat{i} + a_j \hat{j} + a_k \hat{k}, \vec{b} = b_i \hat{i} + b_j \hat{j} + b_k \hat{k}$
...
$$
\det{\begin{pmatrix}\hat{i} & \hat{j} & \hat{k} \\ a_i & a_j & a_k \\ b_i & b_j & b_k\end{pmatrix}} = \hat{i} (a_jb_k - a_kb_j) - \hat{j} (a_ib_k - a_kb_i) + \hat{k} (a_ib_j - a_jb_i)
$$